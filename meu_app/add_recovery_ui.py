...
# Imports needed: signInWithEmailAndPassword from firebase/auth, auth from FirebaseConfig
if "signInWithEmailAndPassword" not in content:
    ...
    if "signInWithEmailAndPassword" not in content:
        # Just inject it near the top
        content = content.replace("import React,", "import { auth } from '@/src/services/FirebaseConfig';\nimport { signInWithEmailAndPassword } from 'firebase/auth';\nimport React,")
...
# Esqueci o PIN button near delete button or at bottom of numeric pad
btn_insert = """
          <TouchableOpacity onPress={() => setShowRecovery(true)} style={{ marginTop: 20 }}>
            <Text style={{ color: theme.textSecondary, fontFamily: 'Inter_400Regular', textDecorationLine: 'underline' }}>Esqueci o PIN</Text>
          </TouchableOpacity>
"""
if "Esqueci o PIN" not in content:
